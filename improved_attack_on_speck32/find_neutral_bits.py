...
def cal_neutrality(n=2**20, diff_in=(0x8020, 0x4101), diff_out=(0x0040, 0x0), nr=3):
    neutrality = np.zeros(word_size*2, dtype=np.float)
    ...
    for i in range(n):
        ...
        if c0l ^ c1l == diff_out[0] and c0r ^ c1r == diff_out[1]:
            ...
            for j in range(word_size):
                ...
                if d0l ^ d1l == diff_out[0] and d0r ^ d1r == diff_out[1]:
                    neutrality[j+word_size] = neutrality[j+word_size] + 1
...
def verify_bit_set_neutrality(n=2**24, diff_in=(0x8020, 0x4101), diff_out=(0x40, 0x0), nr=3, bits=None):
    neutrality = np.zeros(len(bits), dtype=np.float)
    print('the input difference is ', (hex(diff_in[0]), hex(diff_in[1])))
    print('the output difference is ', (hex(diff_out[0]), hex(diff_out[1])))
    print('the number of encryption round is ', nr)
        ...
        if c0l ^ c1l == diff_out[0] and c0r ^ c1r == diff_out[1]:
            ...
            for h in range(len(bits)):
                ...
                for j in cur_set:
                    ...
                    d0 |= d >> 16
                    d1 |= d & 0xffff
                t0l, t0r = p0l ^ d0, p0r ^ d1
                t1l, t1r = p1l ^ d0, p1r ^ d1
                d0l, d0r = sp.encrypt((t0l, t0r), ks)
                d1l, d1r = sp.encrypt((t1l, t1r), ks)
                if d0l ^ d1l == diff_out[0] and d0r ^ d1r == diff_out[1]:
                    neutrality[h] = neutrality[h] + 1
    neutrality = neutrality / num
    print('the neutrality test results are ')
    for j in range(len(bits)):
        print('the bit index is ', bits[j], ' the neutrality is ', neutrality[j])


# prob = 2**(-6)
cal_neutrality(n=2**16, diff_in=(0x211, 0xa04), diff_out=(0x40, 0x0), nr=2)

# prob = 2**(-12)
# cal_neutrality(n=2**24, diff_in=(0x8020, 0x4101), diff_out=(0x40, 0x0), nr=3)

# prob = 2**(-12)
# bits_sets = [[12, 19], [14, 21], [6, 29], [0, 8, 31]]
# verify_bit_set_neutrality(n=2**22, diff_in=(0x8020, 0x4101), diff_out=(0x40, 0x0), nr=3, bits=bits_sets)
